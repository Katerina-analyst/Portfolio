...
|-------------|---------------------|----------------|--------------------|-----------------|--------------------|-------------------------|
| id | Уникальный идентификатор типа процедуры | bigint | — | PRIMARY KEY, AUTO_INCREMENT | NOT NULL | Положительное целое число, генерируется системой |
| name | Название направления процедуры | varchar | 255 | UNIQUE | NOT NULL | Например: уходовые процедуры, аппаратные процедуры, инъекционные процедуры |
...
| type_id | Идентификатор направления процедуры | bigint | — | FOREIGN KEY к Тип процедуры.id | NOT NULL | Значение из справочника типов процедур |
| master_user_id | Идентификатор мастера, который выполняет процедуру | bigint | — | FOREIGN KEY Пользователь.id | NOT NULL | Пользователь с `user_type = master` |
...
| procedure_id | Идентификатор процедуры, на которую записан клиент | bigint | — | FOREIGN KEY к Процедура.id | NOT NULL | Существующая процедура со статусом `available` |
| client_user_id | Идентификатор клиента, который записан на процедуру | bigint | — | FOREIGN KEY к Пользователь.id | NOT NULL | Пользователь с `user_type = client` |
| schedule_day_id | Идентификатор дня расписания, в рамках которого создана запись | bigint | — | FOREIGN KEY к День расписания.id | NOT NULL | Существующий рабочий день расписания |
...
| master_user_id | Идентификатор мастера, который выполняет процедуру | bigint | — | FOREIGN KEY к Пользователь.id | NOT NULL | Пользователь с `user_type = master` |
...
| schedule_day_id | Идентификатор дня расписания | bigint | — | FOREIGN KEY к День расписания.id | NOT NULL | Существующий рабочий день расписания с `is_working = true` |
...
| booking_id | Идентификатор записи | bigint | — | FOREIGN KEY к Запись.id | NOT NULL | Существующая запись на процедуру |
| recipient_user_id | Идентификатор получателя | bigint | — | FOREIGN KEY к Пользователь.id | NOT NULL | Пользователь, которому инициирована отправка уведомления |
...
Сущность хранит контактное значение пользователя для выбранного типа канала связи, признак канала по умолчанию и статус активности.
...
| user_id | Идентификатор пользователя, которому принадлежит канал связи | bigint | — | FOREIGN KEY к Пользователь.id | NOT NULL | Существующий пользователь |
| channel_type_id | Идентификатор типа канала связи | bigint | — | FOREIGN KEY к Тип канала связи.id | NOT NULL | Существующий тип канала связи |
